...
public class PostDtoDaoImpl implements PostDtoDao {
    ...
    public List<PostDto> getAllPosts() {
        List<PostDto> postDtoList = entityManager.createQuery("select " +
                ...
                "t.id," +
                "t.text, " +
                "(select count(bm.id) from bm where bm.post.id = p.id), " + //FIXME жопа
                "(select count(l.id) from l where l.post.id = p.id), " +
                "(select count(c.id) from c where c.post.id = p.id), " +
                "p.repostPerson.size " +
                "from Post as p " +
                "join p.user as u " +
                "join p.media as m " +
                "left join p.tags as t " +
                "left join Bookmark as bm on bm.post.id = p.id " +
                "left join PostLike as l on l.post.id = p.id " +
                "left join PostComment as c on c.post.id = p.id ")
                .unwrap(Query.class)
                .setResultTransformer(new ResultTransformer() {
                    ...
                    public Object transformTuple(Object[] objects, String[] strings) {
                        ...
                        return PostDto.builder()
                                ...
                                .shareAmount((Integer) objects[16])
                                .build();
                    }

                    @Override
                    public List transformList(List list) {
                        Map<Long, PostDto> result = new TreeMap<>(Comparator.naturalOrder());
                        for (Object obj : list) {
