...
@Component("getPostsByTag")
public class PageDtoGetPostsByTagDaoImpl implements PageDtoDao<PostDto> {
    @PersistenceContext
    private EntityManager entityManager;
    private final PostDtoDao postDtoDao;

    @Autowired
    public PageDtoGetPostsByTagDaoImpl(PostDtoDao postDtoDao) {
    ...
    public Long getCount(Map<String, Object> parameters) {
        return entityManager.createQuery(
                "select count (p) from Post p join p.tags as t where t.text = : tagText",
                Long.class
        ).setParameter("tagText", parameters.get("tagText"))
                .getSingleResult();
    }
