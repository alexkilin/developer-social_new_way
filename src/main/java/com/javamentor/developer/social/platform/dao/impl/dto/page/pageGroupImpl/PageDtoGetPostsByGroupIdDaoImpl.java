...
public class PageDtoGetPostsByGroupIdDaoImpl implements PageDtoDao<GroupWallDto> {
    @PersistenceContext
    ...
    @Autowired
    public PageDtoGetPostsByGroupIdDaoImpl(GroupDtoDao groupDtoDao, PostDtoDao postDtoDao) {
    ...
    public List<GroupWallDto> getItems(Map<String, Object> parameters) {
        ...
        groupWallDtoList.forEach(groupWallDto -> {
            groupWallDto.setTags(postDtoDao.getTagsByPostId(groupWallDto.getId()));
            groupWallDto.setMedia(postDtoDao.getMediasByPostId(groupWallDto.getId()));
        });
