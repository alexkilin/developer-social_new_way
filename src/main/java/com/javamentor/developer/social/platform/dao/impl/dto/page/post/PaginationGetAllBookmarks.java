...
@Component("getAllBookmarks")
public class PaginationGetAllBookmarks implements PaginationDao<PostDto> {
    @PersistenceContext
    private EntityManager entityManager;

    public PaginationGetAllBookmarks() {
    }
    ...
    public List<PostDto> getItems(Map<String, Object> parameters) {
        ...
        return (List<PostDto>) entityManager.createQuery(
                "select p.id, " +
                        ...
                        "case when exists " +
                        "(select pl from PostLike as pl where pl.post.id = p.id)" +
                        "then true else false end," +
                        "case when exists " +
                        "(select bm from Bookmark as bm where bm.post.id = p.id)" +
                        "then true else false end," +
                        "case when exists " +
                        "(select rp from Repost as rp where rp.post.id = p.id)" +
                        "then true else false end " +
                        "from Bookmark as bm " +
                        "join bm.user as u " +
                        "join bm.post as p " +
    ...
    public Long getCount(Map<String, Object> parameters) {
        return entityManager.createQuery(
                "select count (bm) from Bookmark bm",
                Long.class
        ).getSingleResult();
    }
}
