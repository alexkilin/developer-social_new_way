...
@Component("getPostsByUserId")
public class PageDtoGetPostsByUserIdDaoImpl implements PageDtoDao<PostDto> {
    @PersistenceContext
    private EntityManager entityManager;
    private final PostDtoDao postDtoDao;

    @Autowired
    public PageDtoGetPostsByUserIdDaoImpl(PostDtoDao postDtoDao) {
    ...
    public Long getCount(Map<String, Object> parameters) {
        return entityManager.createQuery(
                "select count (p) from Post p where p.user.userId = :userId",
                Long.class
        ).setParameter("userId", parameters.get("userId"))
                .getSingleResult();
    }
