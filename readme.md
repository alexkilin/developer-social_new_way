...
## FlyWay
ВАЖНО!!! При работе с *Flyway* нужно понимать, что _SQL файлы из директории migration_, которые уже накатили на _Базу Данных (БД)_ изменять нельзя. Будет расхождение по контрольной сумме, которые Flyway запоминает с помощью специальных таблиц.     
Если такое все таки произошло и все сломалось - дропните все таблицы. Если Вы создаёте новую сущность или хотите произвести любые манипуляции с БД, для безопасной работы с БД необходимо создавать новые SQL файлы в migration. 
Именовать файлы необходимо строго по правилам. НАПРИМЕР: *V2021.02.16.14.00_ _название вашего файла* , где после буквы V идет дата в формате "год.месяц.дата.час.минута", далее идет ДВА НИЖНИХ ПОДЧЕРКИВАНИЯ, после которых 
указывается название файла. 
Более подробно ознакомьтесь с Flyway (https://flywaydb.org/documentation/)
