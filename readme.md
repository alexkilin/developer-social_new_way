...
![пример](https://assets.postman.com/postman-docs/autogenerated-auth-header.jpg)
- ключ - *Authorization*
...
Вы так же можете найти мой образ - *solhaime/devsocial_elk:tag* - на данный момент есть две версии 3.0 и 4.0. Сейчас используется 3.0.

Что делать, если ELK стак упал:
1) `docker run —name elk -p 5601:5601 -p 9200:9200 -p 5044:5044 -d solhaime/devsocial_elk:3.0`
2) запускаем компоус приложения
3) `docker network connect developer-social_default elk`
4) Если елк стак упал, то реквизиты сбросились до elastic/changeme, проделываем шаг 1 и 3, заходим в кибану и меняем пароль.
5) Если ELK не получает логи после перезапуска приложения и бд, делаем шаг 3. Не помогло? - возможно упал один из сервисов. Полная перезагрузка и шаг 4. Не помогло? - проблема не в ЕЛК.
