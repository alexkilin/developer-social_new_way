...
Вы так же можете найти мой образ - *solhaime/devsocial_elk:3.0* - на данный момент есть две версии 3.0 и 4.0. Сейчас используется 3.0.
...
Если не помогло:
1) `docker rm -f elk`
2) `docker run —name elk -p 5601:5601 -p 9200:9200 -p 5044:5044 -d solhaime/devsocial_elk:3.0`
3) `docker network connect developer-social_default elk`
- Если елк стак упал, то реквизиты сбросились до elastic/changeme, после перезапуска заходим в кибану и меняем пароль.
...
[***_Актуальная диаграмма отношения сущностей (ERD - Entity-Relationship Diagram)_***](https://dbdiagram.io/d/600f2d3b80d742080a37cd58)
